function solve() {
   ...
   function onClick() {
      ...
      for (let row of rows) {                         // Минаваме по всеки ред от редовете 
         ...
         if (row.textContent.includes(input.value)) {   // innerHTML проверява във всички деца на елемента, който сме избрали 
            row.classList.add("select");                 // Ако намери на самият ред слагаме className = select 
         }
      }
