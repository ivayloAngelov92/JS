...
gameController.get('/:id/details', async (req, res) => {
	try {
		...
		game.hasBought = game.buyers.some(id => id._id == req.user?._id); //checks if publication has been shared by current userID
...
gameController.get('/:id/buy', hasUser(), async (req, res) => {
	try {
		...
		if (game.owner._id != req.user._id && game.buyers.some(id => id == req.user._id) == false){  //checks if publication has been shared/liked/bought by current userID  
			await buy(game, req.user._id); // req.user._id == current user
    	}
